...
public class UserServiceImpl implements UserService {
	...
	public void upgradeLevels(UserVO inVO) throws SQLException {
		 ...
		 */
		 ...
		 //SILVER(30회 이상 추천) -> GOLD
		 //GOLD는 대상이 아님
		 
		 for(UserVO user :list) {
			 boolean changeLevel = false;
			 
			 if(user.getLevel() == Level.BASIC && user.getLogin() >= 50) {//BASIC(50회 이상 로그인) -> SILVER
				 changeLevel = true;
				 user.setLevel(Level.SILVER);
			 }else if(user.getLevel() == Level.SILVER && user.getRecommend() >= 30) {//SILVER(30회 이상 추천) -> GOLD
				 changeLevel = true;
